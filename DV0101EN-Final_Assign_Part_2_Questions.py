...
app.layout = html.Div([
    ...
    html.Div(dcc.Dropdown(
            ...
            value='year_list = [i for i in range(1980, 2024, 1)]',
            style={width:80, padding:3, font-size:20, textalign:center}
        )),
...
def update_input_container(select_year,selected_statistics):
...
@app.callback(
    ...
    [Input(component_id='select_year', component_property='value'), Input(component_id='dropdown-statistics', component_property='value')])

def update_output_container(select-year,selected_statistics):
...
# Plot 4 bar chart for the effect of unemployment rate on vehicle type and sales
        urate_rec = recession_data.groupby('Vehicle_Type')['Unemployment_Rate']   
        R_chart4  = dcc.Graph(figure=px.bar(urate_rec,
                            x="Vehicle_Type",
                            y="Unemployment_Rate",
        ...
        return [
            html.Div(className='chart-item', children=[html.Div(children=R_chart1)],style={'display': 'flex'}),
...
# Plot bar chart for average number of vehicles sold during the given year
        ...
        Y_chart3 = dcc.Graph(figure=px.bar(avr_vdata,
                                  ...
                                  title='Average Vehicles Sold by Vehicle Type in the year {select-year}'.format(input_year)))

# Total Advertisement Expenditure for each vehicle using pie chart
        exp_data=yearly_data.groupby(Vehicle_Type)['Advertising_Expenditure'].sum().reset_index()
        Y_chart4 = dcc.Graph(figure=px.bar(exp_data,
...
#TASK 2.6: Returning the graphs for displaying Yearly data
        return [
             html.Div(className='chart-item', children=[html.Div(children=Y_chart1)],style={'display': 'flex'}),
             html.Div(className='chart-item', children=[html.Div(children=Y_chart2)],style={'display': 'flex'}),
